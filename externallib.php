...
/**
 * External API for onesignal web services
 *
 * @package    message_onesignal
 * @category   external
 * @copyright  2012 Jerome Mouneyrac <jerome@moodle.com>
 ...
 * @since Moodle 2.7
 */
class message_onesignal_external extends external_api {
    ...
    public static function is_system_configured() {
        ...
        $processor = $DB->get_record('message_processors', array('name' => 'onesignal'), '*', MUST_EXIST);
        if (!$processor->enabled) {
            return 0;
        }

        // Then, check if the plugin is completly configured.
        $manager = new message_onesignal_manager();
    ...
    /**
     * Check if the users have notification preferences configured for the onesignal plugin
     *
    ...
    public static function are_notification_preferences_configured($userids) {
        ...
        foreach ($users as $user) {
            ...
            if ($currentuser or $hasuserupdatecap) {
                ...
                // the user preferences to check if at least one is enabled for onesignal plugin.
                ...
                foreach ($providers as $provider) {
                    ...
                    foreach (array('loggedin', 'loggedoff') as $state) {
                        ...
                        if (in_array('onesignal', $prefstocheck)) {
    ...
    public static function get_user_devices($appid, $userid = 0) {
        ...
        if (!self::is_system_configured()) {
            ...
            // We catch exceptions here because get_user_devices may try to connect to onesignal.
            try {
                $manager = new message_onesignal_manager();
        ...
        return new external_single_structure(
            array(
                'devices' => new external_multiple_structure(
                    new external_single_structure(
                        array (
                            'id' => new external_value(PARAM_INT, 'Device id (in the message_onesignal table)'),
    ...
    public static function enable_device($deviceid, $enable) {
        ...
        $context = context_system::instance();
        self::validate_context($context);
        require_capability('message/onesignal:managedevice', $context);

        if (!message_onesignal_manager::enable_device($params['deviceid'], $params['enable'])) {
            throw new moodle_exception('unknowndevice', 'message_onesignal');
        }
